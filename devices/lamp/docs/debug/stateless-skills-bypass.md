...
- `motion.activity` (wellbeing) và `emotion.detected` (music-suggestion) fire từ HAL → Lamp `/sensing` POST → `SendChatMessageWithRun` (`os/services/internal/openclaw/service_chat.go:88`) → OpenClaw `chat.send` RPC.
