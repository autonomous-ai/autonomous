...
|---------|-------|----------|----------------|
| **Hobbyist** | Dev đơn lẻ, dự án cuối tuần | Cho đèn làm trò vui từ laptop / Claude Desktop | HAL HTTP, MCP, CLI |
...
|----------|---------|-----------|
| API token auth cho HAL + Lamp | Không có thì dev phải SSH vào thiết bị mãi mãi | `os/services/server`, `os/hal/server.py` |
...
| OpenAPI spec | Đầu vào cho codegen SDK | HAL tự sinh từ FastAPI; Lamp cần swaggo / kin-openapi |
...
- **Đóng băng API nội bộ**: trước khi P1 ship, khoá v1 endpoint của HAL / Lamp. Nội bộ chạy trên `/internal/*`, public chỉ tiêu thụ `/v1/*`. Sẽ chậm velocity nội bộ — chấp nhận trade-off.
...
Chỉ Phase 0:
1. Plumb API token cho HAL + Lamp (5001 + 5000).
2. Bind LAN + sửa systemd unit.
3. Migrate route sang prefix `/v1/`.
4. Commit OpenAPI spec (HAL tự sinh, Lamp viết tay).
