...
`hermes-gateway` **chỉ khi** config thật sự đổi (guard bằng content-hash — không
restart loop). Hash phát hiện thay đổi bao trùm **cả** `config.yaml` **và** `.env`
(`hermesEnvFile` = `/root/.hermes/.env`); trước đây chỉ hash `config.yaml`, nên một
thay đổi chỉ-token-kênh (chỉ đụng `.env`) sẽ không restart gateway. Hash thêm `.env`
nghĩa là thêm kênh live giờ sẽ restart gateway để Hermes server nhận kênh mới. Việc
này khắc ngách: device **boot thẳng vào Hermes**
